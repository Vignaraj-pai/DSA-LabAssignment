...
plt.savefig('dfs_edge_images/dfs0.png')
plt.clf()
...
#     nx.draw_networkx(G, node_color = color_map, with_labels = True,pos=pos , edge_color = colors, width = alphas)
#     plt.savefig(f'dfs_edge_images/dfs{count}'+'.png')
#     count = count + 1
#     plt.clf()
#     print(start, end = " -> ")
...
def dfs(adj_list, start, visited):
    ...
    for i in adj_list[start]:
        if visited[i] == False:
            colors[edges.index((start, i))] = tree_edge
            alphas[edges.index((start, i))] = 3
            nx.draw_networkx(G, node_color = color_map, with_labels = True, pos = pos, edge_color = colors, width = alphas)
            plt.savefig(f'dfs_edge_images/dfs{count}.png')
            plt.clf()
            ...
            if color_map[list(G.nodes).index(i)] == '#999':
                colors[edges.index((start, i))] = back_edge
                alphas[edges.index((start, i))] = 3
                nx.draw_networkx(G, node_color = color_map, with_labels = True, pos = pos, edge_color = colors, width = alphas)
                plt.savefig(f'dfs_edge_images/dfs{count}.png')
                plt.clf()
...
for t in range(count):
    image = imageio.v2.imread(f'dfs_edge_images/dfs{t}.png')
    frames.append(image)
    
imageio.mimsave('./dfs_edge.gif', # output gif
